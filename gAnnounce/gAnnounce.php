...
{
	$data = $wpdb->get_results("SELECT * from ". WP_G_NEWS_ANNOUNCEMENT . " where gNews_status='Yes' and (`gNews_expiration` >= NOW() or `gNews_expiration` = '0000-00-00') ORDER BY rand( )");
}
else
{
	$data = $wpdb->get_results("select * from ". WP_G_NEWS_ANNOUNCEMENT . " where gNews_status='Yes' and (`gNews_expiration` >= NOW() or `gNews_expiration` = '0000-00-00') ORDER BY gNews_order");
}
